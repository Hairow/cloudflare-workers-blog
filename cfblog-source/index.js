...
//将 wrangler.jsonc vars + secrets 合并到 OPT（仅在首次请求或值缺失时）
function mergeEnvConfig(env) {
    OPT.user = env.USER || OPT.user;
    OPT.password = env.PASSWORD || OPT.password;
    OPT.siteDomain = env.SITE_DOMAIN || OPT.siteDomain;
    OPT.privateBlog = env.PRIVATE_BLOG === 'true';
    OPT.siteName = env.SITE_NAME || OPT.siteName;
    OPT.siteDescription = env.SITE_DESCRIPTION || OPT.siteDescription;
    OPT.keyWords = env.KEY_WORDS || OPT.keyWords;
    OPT.pageSize = parseInt(env.PAGE_SIZE) || OPT.pageSize;
    OPT.recentlySize = parseInt(env.RECENTLY_SIZE) || OPT.recentlySize;
    OPT.readMoreLength = parseInt(env.READ_MORE_LENGTH) || OPT.readMoreLength;
    OPT.cacheTime = parseInt(env.CACHE_TIME) || OPT.cacheTime;
    OPT.themeURL = env.THEME_URL || OPT.themeURL;
    OPT.cacheZoneId = env.CACHE_ZONE_ID || OPT.cacheZoneId;
    OPT.cacheToken = env.CACHE_TOKEN || OPT.cacheToken;
}
...
async function handleRequest(request, env, ctx) {
    ...
    if (pathA == "sitemap.xml") {
        ...
        response = new Response(await getSiteMap(env), {
            headers: {
        ...
        //console.log(request);
        //处理html正文
        let html = await getHtml(request, env);
...
//入口判断router
async function getHtml(request, env) {
    let url = new URL(request.url);
    ...
    if (pathA == 'page' && parseInt(pathB) > 0) {//返回一个文章列表页面
        return await GetHtmlPageNew(pathA, parseInt(pathB), env);
    }
    else if (pathA == 'category' && pathB.length > 0) {
        return await GetHtmlTagsNew(pathA, pathB, parseInt(pathC), env);
    }
        ...
        return await GetHtmlSingleNew(pathA, pathB, env);
    }
    else if (pathA == 'search') {

    } else if (pathA == 'admin') {
        return await GetHtmlAdmin(request, path, env);
    }
    else {
        return OPT.html404;
    }
...
//单一文章
async function GetHtmlSingleNew(pathA, pathB, env) {
    ...
    //获取mueme
    let widgetMenuList = await getKVByKey("SYSTEM_VALUE_WidgetMenu", true, env);
    //获取分类
    let widgetCategoryList = await getKVByKey("SYSTEM_VALUE_WidgetCategory", true, env);
    //获取标签
    let widgetTagsList = await getKVByKey("SYSTEM_VALUE_WidgetTags", true, env);
    //获取友情链接
    let widgetLinkList = await getKVByKey("SYSTEM_VALUE_WidgetLink", true, env);
    //获取所有文章
    let articleListAll = await getKVByKey("SYSTEM_INDEX_LIST", true, env);
    //获取最近文章
    ...
    ////////////////////////////////////////////开始有变化///////////////////////////////////
    //获取单篇文章 以及上下文
    let jsonArticleSingleArray = await getKVArticleSingle(pathB, env);//page = pathB
...
//标签+分类 
async function GetHtmlTagsNew(pathA, pathB, pathC, env) {
    pathB = decodeURI(pathB)
    //console.log("进入函数GetHtmlPage");
    //html = themepage.replace(GetHtmlMenu,GetHtmlArticle,GetHtmlRecently,GetHtmlCategory,GetHtmlTags)
    //获取模板
    ...
    ////////////////////////////////////////////开始有变化///////////////////////////////////
    //获取本页文章
    let jsonArticleListArray = await getKVArticleCategory(pathB, pathC, OPT.pageSize, env);//关键字,页码
...
//page的htmlNew
async function GetHtmlPageNew(pathA, pathB, env) {
    //console.log("进入函数GetHtmlPage");
    //html = themepage.replace(GetHtmlMenu,GetHtmlArticle,GetHtmlRecently,GetHtmlCategory,GetHtmlTags)
    //获取模板
...
//admin页面的html
async function GetHtmlAdmin(request, path, env) {
    ...
    if (path.length == 1 || path[1] == "list") {
        ...
        let categoryJson = await getKVByKey("SYSTEM_VALUE_WidgetCategory", true, env);
        let menuJson = await getKVByKey("SYSTEM_VALUE_WidgetMenu", true, env);
        let linkJson = await getKVByKey("SYSTEM_VALUE_WidgetLink", true, env);
    ...
    if (path[1] == "publish") {
        ...
        //提取所有index
        let jsonIndex = await getKVByKey("SYSTEM_INDEX_LIST", true, env);
        let jsonTags = [];
        //提取tag到变量
        ...
        await putKV("SYSTEM_VALUE_WidgetTags", JSON.stringify(jsonTags), env);
        //缓存控件html到数据库
    ...
    if (path[1] == "getList") {
        ...
        let jsonArticleListArray = await getKVArticleIndex(page, 20, env);//page = pathB
    ...
    if (path[1] == "edit") {
        ...
        //替换连个json categoryJson menuJson 
        let categoryJson = await getKVByKey("SYSTEM_VALUE_WidgetCategory", false, env);
        //let menuJson =await getKVByKey("SYSTEM_VALUE_WidgetMenu");
        let articleJson = await getKVByKey(articleId, false, env);
        //console.log(" categoryJson ",typeof categoryJson,categoryJson.toString());
    ...
    if (path[1] == "saveConfig") {
        ...
        if (isJSON(WidgetCategory) && isJSON(WidgetMenu)) {
            await putKV("SYSTEM_VALUE_" + "WidgetCategory", WidgetCategory, env);
            await putKV("SYSTEM_VALUE_" + "WidgetMenu", WidgetMenu, env);
            await putKV("SYSTEM_VALUE_" + "WidgetLink", WidgetLink, env);
    ...
    if (path[1] == "import") {
        ...
        if (isJSON(importJson)) {
            ...
            for (let i = 0; i < keys.length; ++i) {
                ...
                await putKV(keys[i], importJson[keys[i]], env);
            }
    ...
    if (path[1] == "saveAddNew") {
        ...
        if (title.length > 0 && createDate.length > 0 && category.length > 0 && contentMD.length > 0 && contentHtml.length > 0) {
            ...
            //全文写入数据库
            await putKV(id, JSON.stringify(articleFull), env);//JSON.stringify(articleFull)
            let articleSimpel = {
                "id": id,
                "title": title,
                "img": img,
            ...
            let oldIndex = await getKVByKey("SYSTEM_INDEX_LIST", true, env);
            let newIndex = [];
            newIndex.push(articleSimpel);
            newIndex = newIndex.concat(oldIndex);
            newIndex = sortByKey(newIndex, "id")
            //这里要排个序
            await putKV("SYSTEM_INDEX_LIST", JSON.stringify(newIndex), env);
    ...
    if (path[1] == "delete") {
        ...
        if (articleId.length == 6) {
            await env.CFBLOG.delete(articleId);
            ...
            //写入索引数据库
            await putKV("SYSTEM_INDEX_LIST", JSON.stringify(oldIndex), env);
...
async function getSiteMap(env) {
    ...
    let ArticleAll = await getKVByKey("SYSTEM_INDEX_LIST", true, env);
...
///////////////////////////////////// KV操作 //////////////////////////////////////////////////////////////////
//获取文章索引Category
async function getKVArticleCategory(keyWords, page, pageSize, env) {
...
async function getKVArticleSingle(articleId, env) {
    articleId = ("00000" + parseInt(articleId)).substr(-6);
    let ArticleIndexJson = await getKVByKey("SYSTEM_INDEX_LIST", true, env);
    ...
    //获取当前文章
    let ArticleJust = await getKVByKey(articleId, true, env);
...
//获取文章索引
async function getKVArticleIndex(page, pageSize, env) {
    page = (page <= 1 ? 1 : page);
    let ArticleIndexJson = await getKVByKey("SYSTEM_INDEX_LIST", true, env);
...
//返回并设置文章ID
async function getKVNewId(env) {
    let V = await getKVByKey("SYSTEM_INDEX_NUM", false, env);
    if (V === '' || V === null || V === '[]' || V === undefined) {
        await putKV("SYSTEM_INDEX_NUM", 1, env);
        return "000001";

    }
    else {
        await putKV("SYSTEM_INDEX_NUM", parseInt(V) + 1, env);
...
async function getKVByKey(key, isJSON, env) {
    console.log("------------KV读取---------------------:", key, isJSON);
    let V = await env.CFBLOG.get(key);
    if (isJSON) {
        try {
...
async function getKVKeys(env, keys = [], cursor = '', limit = 1) {
    const value = await env.CFBLOG.list({ limit: limit, cursor: cursor })
    keys = keys.concat(value.keys)
    if (value.list_complete) {
        let kvs = { OPT: OPT }
        for (let i = 0; i < keys.length; ++i) {
            const KEYvalue = await env.CFBLOG.get(keys[i].name)
            if (KEYvalue != null) {
        ...
        return await getKVKeys(env, keys, value.cursor, limit);
    }
    return {};
}

async function putKV(key, v, env) {
    if (v == null || v == undefined)
        return false;
    if (typeof v === "object")
        v = JSON.stringify(v)
    return await env.CFBLOG.put(key, v);
}

//发表新文章后,1分钟后,刷新一下SYSTEM_INDEX_LIST and SYSTEM_INDEX_NUM
async function KVRefreshArticleIndex(env) {
    let articleKeys = await env.CFBLOG.list({ "prefix": "0" })
